...
function Write-Fail {
    ...
    Write-Host "    Fix the issue above and re-run install.bat" -ForegroundColor Red
    Write-Host ""
...
Write-Host ""
Write-Host "  Leave any field blank to accept the default value." -ForegroundColor DarkGray
Write-Host ""
...
if (-not (Find-Cmd "psql")) {
    Write-Host "    Downloading PostgreSQL 15 (this takes a few minutes)..." -ForegroundColor DarkGray
...
[System.IO.File]::WriteAllText($envTarget, $envContent, [System.Text.Encoding]::UTF8)
...
$dbCheck = (psql -U postgres -h localhost -p $DbPort -tAc "SELECT 1 FROM pg_database WHERE datname='phq_dashboard';" 2>&1)
$dbExists = ($dbCheck -ne $null) -and ("$dbCheck".Trim() -eq "1")
...
# ── STEP 8: Install PM2 ───────────────────────────────────────────────────────
Write-Step "8" "Installing PM2 process manager globally"
...
Write-Step "14" "Starting application with PM2"
